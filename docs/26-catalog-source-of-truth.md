...
15. **Будущий автосбор не переписывает текущий каталог.** После reminder-слота
    подтверждённая публикация ответственного создаёт/обновляет отдельный
    `scheduled`-каталог. В `opened_at` он заменяет прежний `open`-каталог.
16. **SKU повторяющейся позиции сохраняется между версиями.** Сначала берётся
    SKU последнего каталога того же чата с тем же ключом памяти; новый
    детерминированный SKU создаётся только для действительно новой позиции.

Подробный lifecycle: [[32-parser-health-pipeline]].
Автоматический цикл: [[34-collection-automation]].
