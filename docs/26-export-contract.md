...
> - Fuzzy header assignment для unresolved-строки: она остаётся только в
>   колонке 4, даже если её текст похож на существующий заголовок товара.
...
- Каталожные позиции, которых **нет** в шаблоне → получают новую колонку в конце листа с точным названием из `catalog_items.title` (в порядке position_order → id).
...
| `template-strict` | Из шаблона + **пре-создаются колонки для всех кат. позиций** | Нет: unresolved остаётся только в колонке 4 |
| `template-distribution` | Перезаписывает шаблон точными названиями каталога | Нет |

### Инвариант разрешённой строки и количества

- Сетка заголовков может искать соответствующий catalog item по названию, но
  товарная ячейка заполняется только после разрешения самой order line в этот
  catalog item. Похожий header не является parser decision.
- Для `шт/уп/банка/наб` явное число клиента сохраняется, unit приводится к
  `catalog_items.unit_hint`; различие формулировки дискретной единицы не должно
  обнулить товар.
- Строка без catalog link остаётся видимой в `orders.raw_text` (колонка 4), но
  не участвует в SKU totals до безопасного parser/operator decision.
